...
function generate_left_side_string() {
  ...
  if [ "$transparent" = "true" ]; then
    local separator_end="#[bg=default]#{?client_prefix,#[fg=${PALLETE[yellow]}],#[fg=${PALLETE[teal]}]}${left_separator:?}#[none]"
  else
    local separator_end="#[bg=${PALLETE[bg_highlight]}]#{?client_prefix,#[fg=${PALLETE[yellow]}],#[fg=${PALLETE[teal]}]}${left_separator:?}#[none]"
  fi

  echo "#[fg=${PALLETE[fg_gutter]},bold]#{?client_prefix,#[bg=${PALLETE[yellow]}],#[bg=${PALLETE[teal]}]} ${session_icon} #S ${separator_end}"
}
...
function generate_active_window_string() {
  ...
  if [ "$transparent" = "true" ]; then
    ...
    separator_internal="#[bg=${PALLETE['blue0']},fg=${PALLETE['blue']}]${left_separator:?}#[none]"
    separator_end="#[bg=default,fg=${PALLETE['blue0']}]${left_separator:?}#[none]"
  else
    separator_start="#[bg=${PALLETE['blue']},fg=${PALLETE['bg_highlight']}]${left_separator:?}#[none]"
    separator_internal="#[bg=${PALLETE['blue0']},fg=${PALLETE['blue']}]${left_separator:?}#[none]"
    separator_end="#[bg=${PALLETE[bg_highlight]},fg=${PALLETE['blue0']}]${left_separator:?}#[none]"
  fi
